...
import { customer_source_list, add_customer_source, edit_customer_source, delete_customer_source, get_all_thu, get_totle_thu } from '../../services/thuchiService';
...
class DSPhieuThu extends Component {
    ...
    async getData() {
        if (this.props.admin.roles?.includes('thuchi_list') || this.props.admin.is_admin == 1) {
            ...
            const dataLog = await get_all_thu(d);
            ...
            // console.log(dataS);
            // console.log(this.state.nguonKH);

            const phuthutoday = await get_totle_thu();
            this.setState({ phuthutoday: phuthutoday });
    ...
    async handleOnEndReached() {
        ...
        console.log(from);
        console.log(this.state.stopFetchMore);
        ...
        if (!this.state.stopFetchMore) {
            ...
            var newData = await get_all_thu(d);
            from = from + limit;
                ...
                <TouchableOpacity>
                    <View style={[styles.cardItem, styles.padding10]}>
                        <View style={styles.cardLeft}>
                            <Text style={styles.customer}>{item?.customer_fullname ? item?.customer_fullname : item?.supplier_fullname}</Text>
                            <Text style={styles.txtSP}>TM: {item.tienmat_txt} | CK: {item.chuyenkhoan_txt}</Text>
    ...
    render() {
        const { isEnabled, modalFilterFW, modalFilterFW1, views, lastAddedIndex, alert, titleSource, isModalVisible, nguonKH, data, phuthutoday, loadingMore, stopFetchMore, selectedDateFrom, selectedDateTo } = this.state;
            ...
            <SafeAreaView style={{ flex: 1 }}>
                <View style={styles.container}>
                    <View style={styles.header}>
                        ...
                        <Text style={styles.title}>Danh sách phiểu thu</Text>
                        ...
                        <View style={styles.headerRight}>
                            ...
                            <Modal
                                                ...
                                                <Text style={styles.txtTitle}>Chọn nguồn</Text>
                                                {
                                                    nguonKH.map((view, index) => (
                                                        <SourceComponent
                                                            ...
                                                            gotoCreate={(id, title) => this.gotoCreateThu(id, title)}
                                                        />
                                                    ))
                                                }
                                ...
                                <TouchableOpacity activeOpacity={1} onPress={() => this.setModalFilterFW1(false)}
                                    style={styles.modalBackdrop}
                                />
                            </Modal>
                        </View>
                    ...
                    <View style={styles.txtRight}>
                        <Text style={styles.phuThuChi}>Phụ thu/phụ chi hôm nay: {phuthutoday} đ</Text>
                    </View>
                    <Footer />
                </View >
            </SafeAreaView >
...
export default connect(mapStateToProps, mapDispatchToProps)(DSPhieuThu)
