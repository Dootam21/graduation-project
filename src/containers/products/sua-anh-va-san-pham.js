...
import { get_detail_product, update_product_images } from '../../services/productService';
...
class SuaAnhSanPham extends Component {
    ...
    componentDidMount() {
        this.setState({ u_id: this.props.admin.uid, product_id: this.props.product.id });
    ...
    async getData() {
        ...
        console.log(data);
        if (data.product.images[0].trim() === "") {
            this.setState({ image_show: [], data: data.product });
        }
        else {
            this.setState({ image_show: data.product.images, data: data.product });
        }
    }
    ...
    async saveImages() {
        const d = await update_product_images(this.state);
    ...
    render() {
        const { data, image, image_show, isActive } = this.state;
        const navigation = this.props.navigation;
        return (
            <SafeAreaView style={{ flex: 1 }}>
                <View style={styles.container}>
                    <Header title={'Sửa ảnh và sản phẩm'} />

                    <ScrollView style={{ backgroundColor: "#fff" }}>
                        <View>
                            <Text style={styles.txtChuThich}>Danh sách ảnh và sản phẩm (Giữ và kéo thả để sắp xếp, ấn Lưu sắp xếp để hoàn tất)</Text>
                            ...
                            <ModalConfirm saveImages={() => this.saveImages()} title='Bạn chắc chắn chứ?' txtBtn='Lưu sắp xếp' />
                            <Text style={styles.height10}>

                            </Text>
                            <View style={styles.chonImage}>
                                <Text style={styles.textImage}>Thêm ảnh</Text>
...
//   mapDispatchToProps,
// )(withNavigation(Main));
export default connect(mapStateToProps, mapDispatchToProps)(SuaAnhSanPham)
