...
import { get_nhap_chi_detail, update_nhap_chi } from '../../services/productService';
...
class NhapTraOrderConfirm extends Component {
    ...
    async getData() {
        console.log(this.props?.product?.chi_id);
        const dataLog = await get_nhap_chi_detail({
        ...
        this.setState({ conNo: parseFloat(dataLog?.data?.totle_money?.replace(/,/g, "")) + parseFloat(dataLog?.data?.notruocdon?.replace(/,/g, "")) });
    }
    ...
    handleSubmit() {
        ...
        this.setState({
            data: {
                ...
                chuyenkhoan: this.state.chuyenKhoan == '' || this.state.chuyenKhoan == 0 ? 0 : this.state.chuyenKhoan?.replace(/,/g, ''),
                tienmat: this.state.tienMat == '' || this.state.tienMat == 0 ? 0 : this.state.tienMat?.replace(/,/g, ''),
                phuthu: this.state.phuThu == '' || this.state.phuThu == 0 ? 0 : this.state.phuThu?.replace(/,/g, ''),
                phuchi: this.state.phuChi == '' || this.state.phuChi == 0 ? 0 : this.state.phuChi?.replace(/,/g, ''),
                status: 1,
                ghi_chu: this.state.ghiChu,
                debt: this.state.conNo,
                images: this.state.image_show,
                ngaytrano: ngaytrano,
                chi_id: this.state.data.id,
            }
        }, async () => {
            // console.log(this.state.data);
            // return true
            const dataLog = await update_nhap_chi(this.state.data);
            this.setModalVisible1(false);
            this.setModalVisible2(false);
            this.props.productAction('current_product_id', 0);
            this.props.supplierAction('current_supplier_id', 0);
            this.props.navigation.navigate('DSPhieuNhap');
        })
    }
    ...
    updatePrice() {
        ...
        this.setTienMat(0);
        this.setState({ conNo: parseFloat(this.state?.data?.totle_money.replace(/,/g, "")) + parseFloat(this.state?.data?.notruocdon?.replace(/,/g, "")) });
    }
    ...
    setTongTienNo() {
        ...
        this.setState({ conNo: Number(this.state.tongTienNo) - phuChi - tienMat - chuyenKhoan + phuThu })
    }
    ...
    render() {
        ...
        const { phuChi, phuThu, tienMat, chuyenKhoan, conNo, ghiChu, image_show, ngaytrano } = this.state;
        return (
            <SafeAreaView style={{ flex: 1 }}>
                <View style={styles.container}>
                    <Header title={'Xác nhận toa nhập'} />

                    <ScrollView style={{ backgroundColor: "#fff" }}>
                        <View style={styles.client}>
                            <View style={styles.flexRowCF}>
                                <Text style={styles.attrName}>Nhà cung cấp</Text>
                                <Text style={styles.value}>{data?.user_name}</Text>
                            </View>
                        </View>
                        ...
                        <View>
                            ...
                            <View style={styles.flexRowCF}>
                                ...
                                <Text style={styles.value}>{data?.notruocdon} đ</Text>
                            </View>
                        </View>
                        {
                            conNo > 0 ? (
                            ...
                            <View style={styles.modalContainer2}>
                                <View style={styles.modalContent}>
                                    ...
                                    <View style={styles.btnGroupConfirm}>
                                        ...
                                        <TouchableOpacity style={styles.confirmButton} onPress={() => this.handleSubmit()}>
...
//   mapDispatchToProps,
// )(withNavigation(Main));
export default connect(mapStateToProps, mapDispatchToProps)(NhapTraOrderConfirm)
