...
// import { category_update_status } from '../../actions/category_update_status';
...
import { delete_category, edit_category } from '../../services/categoryService';

class CheckBoxCate extends Component {
    ...
    componentDidMount() {
        const s = this.props.id_status == 1 ? true : false;
        // console.log('CheckBoxCate id:' + this.props.id_cate + ', status: ' + this.props.id_status);
        // console.log(s);
        this.setState({ isSelected: s });
    ...
    setStatus(opt) {
        this.setState({ data: { ...this.state.data, home: opt } });
    }
    setData(opt) {
        this.setState({ data: opt })
    }
    handleCheckBox = (id, _isSelected) => {
        this.setState({
            isSelected: _isSelected,
            data: {
                ...this.state.data,
                home: _isSelected ? 1 : 0,
            }
        });

        this.props.setStatus(id, _isSelected);
    }
    ...
    render() {
        const { data, isSelected, modalDelete, modalEdit, title } = this.state;
        return (
            <TouchableOpacity onPress={() => {
                if (this.props.admin.roles?.includes('category_turn_on_off') || this.props.admin.is_admin == 1) {
                    this.handleCheckBox(data.id, !isSelected)
                }
                else {
                    ...
                    <View style={styles.flexRowC}>
                        <View style={{ marginRight: 15 }}>
                            <View style={styles.checkMark}>
                                {isSelected && <Text style={styles.checkMark1}></Text>}
                            </View>
                        </View>
                        <TouchableOpacity onPress={() => {
...
export default connect(mapStateToProps, mapDispatchToProps)(CheckBoxCate)
