...
import { edit_supplier, get_detail_supplier } from '../../services/supplierService';
...
class EditSupplier extends Component {
    ...
    async getData() {
        ...
        if (data.images && data.images[0])
            this.setState({ image: data.images[0], image_show: data.images });
    ...
    setSelectedCheckbox = (opt) => {
        this.setState({ selectedCheckbox: opt });
    }
    ...
    setPhone = (opt) => {
        this.setState({ data: { ...this.state.data, phone: opt } })
    }
    ...
    handleCheckboxSelect = (index) => {
        this.setSelectedCheckbox(index);
    };

    async btnConFirm() {
        var d = this.state.data;
        d.image_show = this.state.image_show;
        d.loaitien = this.state.selectedCheckbox;
        d.u_id = this.props.admin.uid;

        const dataLog = await edit_supplier(this.state.data);
        console.log(dataLog);
    ...
    getCheckboxStyle = (index) => {
        if (this.state.selectedCheckbox === index) {
            return styles.selectedCheckbox;
        }
    ...
    render() {
        const { data, image, image_show } = this.state;
        return (
                ...
                <View style={styles.container}>
                    <View style={styles.header}>
                        ...
                        <View style={styles.headerCenter}>
                            <Text style={styles.title}>Sửa nhà cung cấp</Text>
                        </View>
                    ...
                    <ScrollView style={{ backgroundColor: "#f5f5f5" }}>
                        <View >
                            <View style={styles.boxAvatar}>
                                <Image style={[styles.uploadImage, styles.avatarImage]} source={{ uri: image }}></Image>
                            </View>
                        ...
                        {/* <View>
                            ...
                            <View style={styles.listAttr}>
                                <View style={styles.attr}>
                                    <Text style={styles.attrName}>Chọn Tỉnh/Thành phố</Text>
                                    <ModalChonAddress txtBtn='Chọn tỉnh' title='Chọn tỉnh thành' />
                                </View>
                                <View style={styles.attr}>
                                    <Text style={styles.attrName}>Chọn Quận/Huyện</Text>
                                    <ModalChonAddress txtBtn='Chọn tỉnh' title='Chọn tỉnh thành' />
                                </View>
                                <View style={styles.attr}>
                                    <Text style={styles.attrName}>Chọn Phường/Xã/Thị trấn</Text>
                                    <ModalChonAddress txtBtn='Chọn tỉnh' title='Chọn tỉnh thành' />
                                </View>
                                <View style={styles.attr}>
                                    <Text style={styles.attrName}>Số nhà/Ngõ xóm</Text>
                                    <TextInput
                        ...
                        {/* <View>
                            <Text style={styles.sectionHeader}>Chọn loại tiền</Text>
                            <View style={styles.btnGroupCurrency}>
                                <TouchableOpacity onPress={() => this.handleCheckboxSelect(0)}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        <Text style={[styles.checkbox1, this.getCheckboxStyle(0)]}>USD</Text>
                                    </View>
                                </TouchableOpacity>

                                <TouchableOpacity onPress={() => this.handleCheckboxSelect(1)}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        <Text style={[styles.checkbox1, this.getCheckboxStyle(1)]}>VND</Text>
                                    </View>
                                </TouchableOpacity>

                                <TouchableOpacity onPress={() => this.handleCheckboxSelect(2)}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        <Text style={[styles.checkbox1, this.getCheckboxStyle(2)]}>RMB</Text>
                                    </View>
                                </TouchableOpacity>

                                <TouchableOpacity onPress={() => this.handleCheckboxSelect(3)}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        <Text style={[styles.checkbox1, this.getCheckboxStyle(3)]}>THB</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                    ...
                    <View style={styles.pd3}>
                        <TouchableOpacity onPress={() => this.setIsShow(true)}>
                            <Text style={styles.btnAdd}>Cập nhật</Text>
                        </TouchableOpacity>
                    </View>
                        ...
                        <View style={styles.modalContainer2}>
                            <View style={styles.modalContent}>
                                ...
                                <View style={styles.btnGroupConfirm}>
                                    <TouchableOpacity style={styles.closeButton} onPress={() => this.setIsShow(false)}>
