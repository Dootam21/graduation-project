...
import { add_supplier } from '../../services/supplierService';
...
const validationSchema = Yup.object().shape({
    title: Yup.string().required('Vui lòng nhập tên nhà cung cấp'),
    phone: Yup.string().required('Vui lòng nhập ít nhất số điện thoại').matches(/^(?!01|04|06)\d{10}$/, 'Số điện thoại không hợp lệ'),
});
...
class AddSupplier extends Component {
    ...
    btnConFirm = async (value) => {
        ...
        const dataLog = await add_supplier(d);
    ...
    render() {
        const { data, image_show } = this.state;
                            ...
                            {({ values, handleChange, handleSubmit, errors, touched, setFieldValue }) => (
                                <View style={styles.groupInputAdd}>
                                    ...
                                    {errors.title && touched.title ? (
                                        <Text style={styles.textValidate}>{errors.title}</Text>
                                    ) : null}
                                    <TextInput
                                        style={styles.inputAdd}
